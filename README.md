...
TaCZ `1.1.5` is optional. When installed, YSM-enabled CustomNPCs holding a TaCZ gun use real TaCZ shooting, aiming, bolting and reloading instead of CustomNPC projectile entities. With Dominion Sword installed, selected gun units also receive the shared **Reload** skill. Separately, every idle gun NPC uses the CNPC configuration's three-stage real reload policy: under 50% after 10 seconds without an enemy, under two thirds after 20 seconds, and any non-full magazine after 30 seconds. The three times are configurable and are normalized to remain strictly ascending; a newly acquired enemy cancels an automatic reload, while a player-issued Reload request remains explicit.
